<template>
  <div class="border border-solid border-gray-200 rounded-sm">
    <div
      class="flex flex-wrap gap-1 p-1 border-b border-gray-200 bg-gray-50 rounded-t-lg"
      v-show="isEditorFocused"
    >
      <Space>
        <Tooltip title="Bold">
          <Button
  ...
  <div class="flex mt-2 gap-2" v-if="props.viewActionButtons && actionButtons">
    <Button type="primary" @click="onSave">Save</Button>
    <Button @click="closeActionButtons">Cancel</Button>
  </div>
</template>
...
const isEditorFocused = ref(false) // Track focus state
...
const closeActionButtons = () => {
    actionButtons.value = false
    isEditorFocused.value = false
}
const showActionButtons = () => {
    actionButtons.value = true
}
...
onMounted(() => {
  editor.value = new Editor({
    ...
    // onBlur: () => (isEditorFocused.value = false), // Update on blur
  })
})
